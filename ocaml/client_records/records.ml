...
let vm_record rpc session_id vm =
	...
	let xgm () = lzy_get guest_metrics in
	{
	  ...
	  fields = [
			...
			make_field ~name:"networks-path-optimized"
				~get:(fun () -> default nid (may (fun m -> string_of_bool m.API.vM_guest_metrics_networks_path_optimized) (xgm ()) )) ();
			make_field ~name:"storages-path-optimized"
				~get:(fun () -> default nid (may (fun m -> string_of_bool m.API.vM_guest_metrics_storages_path_optimized) (xgm ()) )) ();
